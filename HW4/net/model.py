## Based on PromptIR: Prompting for All-in-One Blind Image Restoration
...
class PromptGenBlock(nn.Module):
    def __init__(self, prompt_dim=128, prompt_len=8, prompt_size=96, lin_dim=192):
        ...
        self.linear_layer = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Linear(lin_dim // 2, prompt_len * 2)
        )
        self.conv3x3 = nn.Conv2d(prompt_dim, prompt_dim, kernel_size=3, stride=1, padding=1, bias=False)

    def forward(self, x, de_id=None):
        B, C, H, W = x.shape
        emb = F.adaptive_avg_pool2d(x, 1).squeeze(-1).squeeze(-1)
        ...
        if de_id is not None:  # Training phase
